...
namespace Mod_Hub_Base
{
	...
	public:
		...
		static void StartHook(HMODULE hmoduleDLL);
		void Hook();
	...
	*/
	...
	protected:
		...
		uint64_t	find_pattern(uint64_t i64Address, uint64_t end, BYTE* btMask, const char* szMask);
		uint64_t	virtual_find_pattern(uint64_t address, BYTE* btMask, const char* szMask);
	};
